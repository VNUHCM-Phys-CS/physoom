...
export default function CourseList({course,onSelectionChange,userEvents}) {
      ...
      const courseGroup = useMemo(()=>{
        ...
        let l = [{title:"Pending class",data:[],emptyText:"No action needed"},
    ...
    return (<div className="flex flex-col gap-2">
         <Listbox 
          ...
          disabledKeys={["empty"]}
        >
          {courseGroup.map(cg=><ListboxSection key={cg.title} title={cg.title} showDivider>
            {cg.data.map(({title,location,credit,_id})=>
              <ListboxItem key={_id} 
              description={<div className="flex gap-1"><Chip size="sm" color="primary">{location}</Chip><Chip size="sm" variant="shadow">{credit} credits</Chip></div>}>
                {title}
                </ListboxItem >)}
            {(cg.data.length===0)&&<ListboxItem key="empty">{cg.emptyText}</ListboxItem>}
            </ListboxSection>)}
        </Listbox>
    </div>);
}