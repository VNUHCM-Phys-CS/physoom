...
export const defaultGridNVC = {
    ...
    {label:"Break",timeData:[730,770], disabled:{2:1,3:1,4:1,5:1,6:1,7:1,8:1}},
    ...
    calendar2booking: function(ca){
        return calendar2booking(ca,this.data,true)
    }
};
...
export const defaultGridLT = {
    ...
    {label:"Break",timeData:[710,760], disabled:{2:1,3:1,4:1,5:1,6:1,7:1,8:1}},
    ...
    calendar2booking: function(ca){
        return calendar2booking(ca,this.data,1)
    }
};
...
function getIndex(d,arr){
    ...
    for (let i=0;i<arr.length;i++) {
        if (d<=arr[i].timeData[1]){
            return i + (d-arr[i].timeData[0])/(arr[i].timeData[1] - arr[i].timeData[0]);
        }
    }
    index = arr.length-1;
    return index;
...
function index2time(d,arr,isEnd){
    ...
    return base.timeData[0]+(base.timeData[1]-base.timeData[0])*(d-Math.floor(d));
}

function booking2calendar(booking,time_arr){
    const c = {title: booking.course.title, subtitle:booking.teacher_email, duration:booking.course.credit };
    const {time_slot} = booking;
    c.time_slot = {weekday: time_slot.weekday,
        start_time: time_slot.start_time?getIndex(time_slot.start_time,time_arr):undefined, 
        end_time: time_slot.end_time?getIndex(time_slot.end_time,time_arr):undefined, }
    return c
}
function calendar2booking(ca,booking,time_arr,isEnd){
    const {weekday,start_time,end_time} = ca;
    booking.time_slot = {
        ...booking.time_slot,
        weekday,
        ...
        end_date:moment(+booking.course.start_date).add(booking.course.duration, 'weeks').toDate()
    }
    return booking;
}